...
# NOTA: Asegúrate de que este módulo esté accesible
...
def gamificacion(datos_juego=None):
    ...
    # Restablecemos el juego actual
    st.session_state['juego_actual'] = None
    st.rerun() 

# ------------------------------------------------
# B. PÁGINA: MENÚ PRINCIPAL (Con tu diseño CSS)
# ------------------------------------------------

def mostrar_menu_principal(volver_menu_juegos_callback):
    """Muestra el menú de selección de juegos con tu diseño personalizado (Estilo C)."""
    
    # 1. CSS VIBRANTE (SOLO ZONA PRINCIPAL) - CÓDIGO PROPORCIONADO POR EL USUARIO
    st.markdown("""
    <style>
    ...
    # TÍTULO (Ajustado a color oscuro para que se lea en el gris) - CÓDIGO PROPORCIONADO POR EL USUARIO
    st.markdown("""
    ...
    with col1:
        ...
        if st.button("🧠 TRIVIA\n\n¿Cuánto sabes?", key="btn_card_trivia", use_container_width=True):
            st.session_state['juego_actual'] = 'Trivia' # Usamos "Trivia" para el ruteo
            st.rerun()
    ...
    with col3:
        ...
        if st.button("🤖 ROBOT\n\nLógica & Deducción", key="btn_card_robot", use_container_width=True):
            st.session_state['juego_actual'] = 'Ahorcado' # Usando 'Ahorcado' como ID interna
            st.rerun()

    with col4:
        # SORTEADOR
        # Se mantiene el icono HTML del usuario para mantener el diseño
