...
def juego_robot(volver_menu_juegos):
    """
    Versión migrada y autónoma del Juego del Robot (Ahorcado).
    Gestiona su propio estado de sesión y diseño arcade.
    """
    ...
    with col_back:
        if st.button("🔙 Menú", use_container_width=True, key="robot_btn_back_top"):
            # Limpieza selectiva de estados del robot
            keys_to_clear = [
                'robot_challenges', 'robot_level', 'robot_word', 
                'robot_score', 'robot_errors', 'robot_guesses', 'robot_hint'
            ]
    ...
    st.markdown("""
        ...
        /* Excepción para botones de control superior (Menú/Config) */
    ...
    if 'robot_challenges' not in st.session_state:
        ...
        with col_c1:
            tema_robot = st.text_input("Tema del Reto:", placeholder="Ej: Sistema Solar, Verbos...", key="robot_input_tema")
        with col_c2:
            lista_grados_robot = [
                "1° Primaria", "2° Primaria", "3° Primaria", "4° Primaria", "5° Primaria", "6° Primaria",
                "1° Secundaria", "2° Secundaria", "3° Secundaria", "4° Secundaria", "5° Secundaria"
            ]
            grado_robot = st.selectbox("Grado:", lista_grados_robot, index=5, key="robot_select_grado")
        with col_c3:
            cant_robot = st.slider("Palabras:", 3, 10, 5, key="robot_slider_cant")
    ...
    # --- PANEL DE JUEGO ACTIVO ---
        ...
        baterias_restantes = max(0, max_errores - errores)
        ...
        for letra in palabra:
            # Ignoramos espacios si los hubiera en la palabra generada por IA
            if letra == " ":
                palabra_mostrar += "&nbsp;&nbsp;"
        ...
        # Lógica de fin de nivel o juego
        if ganado:
            ...
            if nivel_idx < total_niveles - 1:
                if st.button("➡️ Siguiente Nivel", type="primary", use_container_width=True):
                    ...
                    st.session_state['robot_guesses'] = set()
                    st.session_state['robot_errors'] = 0 # Reset de errores por nivel o mantener? (Aquí resetea)
                    st.rerun()
            else:
                st.balloons()
                ...
                if st.button("🔄 Nueva Misión", type="primary", use_container_width=True):
                    if 'robot_challenges' in st.session_state: del st.session_state['robot_challenges']
                    st.rerun()
            ...
            for i, letra in enumerate(letras_teclado):
                ...
                type_btn = "secondary"
                # Si ya se adivinó y es correcta, resaltar en azul (primary)
