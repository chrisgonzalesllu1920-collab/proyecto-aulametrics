...
COLORS_NIVELES = {
    'AD': '#008450', # Verde oscuro (Logro destacado)
    'A': '#32CD32',  # Verde lima (Logrado)
    'B': '#FFB900',  # Dorado/Amarillo (En proceso)
    'C': '#E81123'   # Rojo (Inicio)
}
...
def mostrar_analisis_general(results):
    ...
    for i, (sheet_name, result) in enumerate(results.items()):
        with tabs[i]:
            ...
            # --- GRÁFICOS INTERACTIVOS (POWER BI LOOK & FEEL) ---
            ...
            if selected_comp:
                ...
                # Definición de color para consistencia
                color_seq = [COLORS_NIVELES[n] for n in df_plot['Nivel']]
                ...
                elif st.session_state.chart_type == 'Embudo de Logros':
                    fig = px.funnel(df_plot, x='Estudiantes', y='Nivel', color='Nivel',
                ...
                # Estilo de Layout común para todos los gráficos (Power BI Style)
                fig.update_layout(
            ...
            # --- PANEL DE ASISTENCIA PEDAGÓGICA ---
...
def mostrar_analisis_por_estudiante(df_first, df_config, info_areas):
    ...
    # Detección de columna de nombre (Lógica original intacta)
    ...
    if estudiante_sel:
        ...
        # Botón de Reporte (Estilo Word corporativo)
        doc_buffer = pedagogical_assistant.generar_reporte_estudiante(estudiante_sel, total_conteo, desglose_areas)
        st.download_button(label="📥 Descargar Informe Individual (Formato Word)", data=doc_buffer, 
                       file_name=f"Informe_Academico_{estudiante_sel}.docx", use_container_width=True, key=f"dl_word_{estudiante_sel}")
        st.markdown("</div>", unsafe_allow_html=True)

@st.cache_data
def convert_df_to_excel(df, area_name, general_info):
    """Función de Excel original con formato mejorado"""
    output = io.BytesIO()
...
def configurar_uploader():
    """Zona de carga con estilo PBI Card"""
    ...
    if uploaded_file:
        with st.spinner('Sincronizando datos con el motor de análisis...'):
...
def inject_pbi_css():
    """CSS para emular Power BI Desktop"""
    st.markdown(f"""
        <style>
